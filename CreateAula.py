...
def criar_aula(id_minicurso: int, titulo: str = Form(...), arquivo: UploadFile = File(None), link: str = Form(None), db: Session = Depends(get_db)):
    ...
    if not arquivo and not link:
        raise HTTPException(status_code=400, detail="Envie um arquivo ou um link para o conteúdo da aula.")
